...
///
/// When a BleatCoreMessage is sent to the server, it may take an indeterminate
...
#[derive(Debug, Clone)]
pub struct BleatCoreFutureState<T> {
    reply_msg: Option<T>,
    waker: Option<Waker>,
}
...
impl<T> Default for BleatCoreFutureState<T> {
    fn default() -> Self {
        BleatCoreFutureState::<T> {
            reply_msg: None,
            waker: None,
        }
    }
...
/// Shared [BleatCoreFutureState] type.
///
/// [BleatCoreFutureState] is made to be shared across futures, and we'll
/// never know if those futures are single or multithreaded. Only needs to
/// unlock for calls to [BleatCoreFutureState::set_reply].
pub type BleatCoreFutureStateShared<T> = Arc<Mutex<BleatCoreFutureState<T>>>;

/// [Future] implementation for [BleatCoreMessageUnion] types send to the server.
///
/// A [Future] implementation that we can always expect to return a
/// [BleatCoreMessageUnion]. Used to deal with getting server replies after
/// sending [BleatCoreMessageUnion] types via the client API.
#[derive(Debug)]
pub struct BleatCoreFuture<T> {
    /// State that holds the waker for the future, and the [BleatCoreMessageUnion] reply (once set).
    ///
    /// ## Notes
    ///
    ...
    waker_state: BleatCoreFutureStateShared<T>,
}

impl<T> Default for BleatCoreFuture<T> {
    fn default() -> Self {
        BleatCoreFuture::<T> {
            waker_state: BleatCoreFutureStateShared::<T>::default(),
        }
    }
}

impl<T> BleatCoreFuture<T> {
    /// Returns a clone of the state, used for moving the state across contexts
    /// (tasks/threads/etc...).
    pub fn get_state_clone(&self) -> BleatCoreFutureStateShared<T> {
...
impl<T> Future for BleatCoreFuture<T> {
    type Output = T;

    /// Returns when the [BleatCoreMessageUnion] reply has been set in the
    /// [BleatCoreFutureStateShared].
